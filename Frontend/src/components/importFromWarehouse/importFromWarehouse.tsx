...
function ImportFromWarehouse({ isOpen, onClose, storeId }: props) {
  ...
  const importFromWarehouse = async () => {
    try {
      ...
      showSuccessToast(t("popups.importFromWarehouse.successToast"));
      onClose();
    } catch (error) {
      console.log(error);
    }
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-start justify-between p-6 pb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="regular text-[10px] text-primary-200 tracking-[0.2em] uppercase leading-tight max-w-[140px] rtl:text-right">
                  {t("popups.importFromWarehouse.subtitle")}
                </span>
              </div>
            ...
            <div className="px-6 pb-6">
              <h2 className="header text-3xl font-bold text-white uppercase leading-tight tracking-wide mb-6 rtl:text-right">
                {t("popups.importFromWarehouse.title")}
              </h2>
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2 relative z-10">
                  <label className="regular text-[10px] text-primary-200 uppercase tracking-[0.1em] rtl:text-right">
                    {t("popups.importFromWarehouse.chooseItem")}
                  </label>
                  <select
                  ...
                  >
                    <option value="" disabled hidden>{t("popups.importFromWarehouse.chooseItemPlaceholder")}</option>
                    {items.map((inventory) => (
                      <option
                ...
                <div className="flex flex-col gap-2 relative z-10">
                  <label className="regular text-[10px] text-primary-200 uppercase tracking-[0.1em] rtl:text-right">
                    {t("popups.importFromWarehouse.sourceWarehouse")}
                  </label>
                  <select
                    ...
                    className="regular w-full bg-[#131C2A] border border-white/20 rounded-[4px] p-4 text-white focus:outline-none appearance-none text-sm cursor-pointer uppercase transition-colors hover:border-white/40 rtl:text-right"
                  >
                    <option value="" disabled hidden>{t("popups.importFromWarehouse.chooseSourcePlaceholder")}</option>
                ...
                <div className="flex flex-col gap-2 relative z-10">
                  <label className="regular text-[10px] text-primary-200 uppercase tracking-[0.1em] rtl:text-right">
                    {t("popups.importFromWarehouse.enterQuantity")}
                  </label>
                  <div className="relative">
                    <input
                      ...
                      placeholder={t("popups.importFromWarehouse.quantityPlaceholder")}
              ...
              >
                {t("popups.importFromWarehouse.executeImport")}
